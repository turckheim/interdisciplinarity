...
print(system.time({
  ...
  for (category in categories) {
    ...
    tmp_wos_nbcat AS (
      SELECT CLEUT_CITANT, CLEUT_CITE, count(*) AS nbCat
      FROM tmp_wos_valides
      GROUP BY CLEUT_CITANT, CLEUT_CITE
    )
    SELECT tmp_wos_valides.CLEUT_CITANT, tmp_wos_valides.CAT_COD_CITE , sum(1/nbCat) AS count
    ...
    GROUP BY (tmp_wos_valides.CLEUT_CITANT, tmp_wos_valides.CAT_COD_CITE)
    ORDER BY tmp_wos_valides.CLEUT_CITANT, tmp_wos_valides.CAT_COD_CITE
    "))
